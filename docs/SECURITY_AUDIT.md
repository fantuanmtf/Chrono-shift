# Chrono-shift 安全审计报告 v3.2.0

> **归档**：历史文档，仅作记录参考，不代表当前实现。当前文档见 docs/ 与 README。


> 审计日期: 2026-05-10 | 代码审计 + CVE 全量扫描 347,868 条 (1999-2026)

## 概览

| 严重度 | 代码审计 | CVE交叉 |
|--------|---------|---------|
| CRITICAL | 4 (已修复) | 358 条匹配 |
| HIGH | 5 (已修复) | 1,484 条匹配 |
| MEDIUM | 6 (4已修复) | - |
| LOW | 4 (已修复) | - |
| **总计** | **19 → 0剩余** | **5,622 条重合** |

> CVE 交叉审计详情: [CVE_AUDIT.md](CVE_AUDIT.md) — 扫描 99,460 CVEs, 13 个依赖库
...
| 机制 | 实现 | 文件 |
|------|------|------|
| i2pd 完整性校验 | SHA-256 首次 seal + 后续 verify | `src/i2p/IntegrityCheck.cpp` |
| 进程防重复 | CreateToolhelp32Snapshot | `src/i2p/I2pdEmbedded.cpp` |
| 端口检测 | is_port_open(7656) | `src/i2p/I2pdEmbedded.cpp` |
| 防篡改回退 | 哈希不匹配 → 拒绝i2p → 推荐Tor | `src/i2p/I2pdEmbedded.cpp` |
| Tor ControlPort 认证 | CookieAuthentication 1 | `src/tor/TorEmbedded.cpp` |
| Tor SafeSocks | SafeSocks 1 + DNSPort 0 | `src/tor/TorEmbedded.cpp` |
